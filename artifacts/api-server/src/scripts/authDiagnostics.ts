...
Usage:
  ...
  node dist/scripts/authDiagnostics.mjs --email business@demo.com --password demo123
...
Notes:
  ...
  --reset-demo-passwords also resets existing demo users to password "${DEMO_PASSWORD}".`);
}
...
async function ensureDemoUsers(resetPasswords: boolean) {
  ...
  console.log(JSON.stringify({ demoPassword: DEMO_PASSWORD, results }, null, 2));
}
...
async function main() {
  ...
  if (args.ensureDemoUsers) {
    await ensureDemoUsers(args.resetDemoPasswords);
  }
