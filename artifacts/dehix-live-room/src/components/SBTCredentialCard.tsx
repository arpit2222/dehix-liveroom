          ...
          {!compact && (
            <div className="flex items-center gap-4 mt-2">
              <div className="text-xs text-muted-foreground">
                <span className="text-foreground/70 font-medium">{credential.githubScore}</span> GitHub
              </div>
              <div className="text-xs text-muted-foreground">
                <span className="text-foreground/70 font-medium">{credential.interviewScore}</span> Interview
              </div>
              <div className="text-xs text-muted-foreground">
                <span className="text-foreground/70 font-medium">{credential.projectsCompleted}</span> Projects
              </div>
            </div>
          )}
        </div>
        <ReputationRing score={credential.reputationScore} size={compact ? 48 : 56} />
      </div>
    </div>
  );
}
