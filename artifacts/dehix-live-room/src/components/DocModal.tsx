    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-background/95 backdrop-blur-md animate-fadeIn">
      ...
      <div className="shrink-0 border-b border-border/40 bg-card/85 px-6 py-4 flex items-center justify-between gap-4 shadow-sm">
        <div className="flex items-center gap-3 min-w-0">
          <div className="flex items-center gap-2 text-primary shrink-0">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest hidden sm:inline">Document Preview</span>
          </div>
          <span className="text-border/40 hidden sm:inline">·</span>
          <span className="font-bold text-sm truncate text-foreground">{doc.title}</span>
      ...
      <div className="flex-1 overflow-y-auto bg-muted/40 dark:bg-muted/10 py-8 px-4 flex justify-center">
        <div className="bg-card text-foreground shadow-xl border border-border/30 rounded-xl p-8 md:p-12 w-full max-w-3xl min-h-[1000px] flex flex-col justify-between">
          ...
          <div className="border-b border-border/20 pb-6 mb-6">
            ...
            <h1 className="text-2xl md:text-3xl font-extrabold text-foreground tracking-tight leading-tight">{doc.title}</h1>
          </div>

          {/* Document Text Body */}
          <div className="flex-1 my-4 overflow-x-hidden">
            {parsedJson ? (
              <JsonDocumentRenderer data={parsedJson} documentType={doc.documentType} />
            ) : (
      ...
      <div className="shrink-0 border-t border-border/40 px-6 py-3 flex items-center justify-between bg-card/85 text-[10px] text-muted-foreground/60 shadow-inner">
        ...
        <div className="flex items-center gap-3">
          <span className="font-semibold cursor-pointer hover:text-foreground" onClick={copy}>Copy to clipboard</span>
          <span>·</span>
          <span className="font-semibold cursor-pointer hover:text-foreground" onClick={download}>Download .txt</span>
        </div>
      </div>
    </div>
  );
  ...
  while (i < lines.length) {
    ...
    if (trimmed.startsWith("|")) {
      ...
      if (rawRows.length > 0) {
        ...
        if (rows.length > 0 && rows[0].every(col => /^-+:*|-*:*|-*$/.test(col))) {
...
function parseInline(text: string): ReactNode[] {
  ...
  return parts.map((part, idx) => {
    if (part === "@dehixai") {
      return (
        <span key={idx} className="font-bold text-primary dark:text-violet-400 underline underline-offset-2 decoration-1">
          @dehixai
        </span>
      );
    }
    ...
    if (part.startsWith("`") && part.endsWith("`")) {
      return (
        <code key={idx} className="rounded px-1.5 py-0.5 text-[12px] font-mono font-semibold bg-muted border border-border/30 text-foreground/90">
          {part.slice(1, -1)}
        </code>
      );
    }
    ...
    <div className="space-y-4 text-foreground/90 leading-relaxed font-sans text-sm selection:bg-primary/10">
      {blocks.map((block, index) => {
        switch (block.type) {
          ...
          case "paragraph":
            return (
              <p key={index} className="text-foreground/80 font-normal leading-relaxed text-justify md:text-left mb-3">
                {parseInline(block.lines.join(" "))}
              </p>
            );
          case "hr":
            return <hr key={index} className="my-6 border-t border-border/35 dark:border-border/15" />;
          case "blockquote":
            return (
              <blockquote key={index} className="border-l-4 border-primary/50 bg-muted/20 px-4 py-2.5 my-3 italic rounded-r text-muted-foreground/90 font-medium">
          ...
          case "code":
            return (
              <pre key={index} className="bg-muted border border-border/30 dark:border-border/15 rounded-lg p-4 font-mono text-xs overflow-x-auto my-4 text-foreground/90 shadow-inner">
                <code>{block.content.join("\n")}</code>
              </pre>
            );
          case "table": {
            return (
              <div key={index} className="overflow-x-auto my-5 border border-border/40 rounded-lg shadow-sm animate-fadeIn">
                  ...
                  <thead className="bg-muted/40 font-bold text-foreground">
                    <tr>
                      {block.headers.map((header, hIdx) => {
                        ...
                        return (
                          <th key={hIdx} className={`px-4 py-2.5 font-semibold text-foreground border-r border-border/10 last:border-0 text-${align}`}>
                            {parseInline(header)}
                          </th>
                        );
                      })}
                    ...
                    {block.rows.map((row, rIdx) => (
                      <tr key={rIdx} className="hover:bg-muted/15 transition-colors">
                        {row.map((cell, cIdx) => {
                          ...
                          return (
                            <td key={cIdx} className={`px-4 py-2 text-foreground/80 border-r border-border/10 last:border-0 text-${align}`}>
                              {parseInline(cell)}
                            </td>
                          );
                        })}
...
function renderExecutiveSummary(execData: any) {
  ...
  const { idea_name, one_line_description, business_goal, target_market, recommended_launch_strategy } = execData;
  return (
        ...
        {business_goal && (
          <div className="space-y-1.5 pt-3 border-t border-border/40">
            <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-1.5">
              <TrendingUp className="h-3.5 w-3.5 text-primary" /> Core Business Goal
            </h3>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {target_market && (
          <div className="rounded-xl border border-border bg-card p-4 space-y-1">
            <span className="text-[10px] font-extrabold uppercase tracking-wider text-muted-foreground">Target Market</span>
        ...
        {recommended_launch_strategy && (
          <div className="rounded-xl border border-border bg-card p-4 space-y-1">
            <span className="text-[10px] font-extrabold uppercase tracking-wider text-muted-foreground">Launch Strategy</span>
            <div className="text-xs font-bold text-foreground flex items-center gap-1.5 mt-1">
          ...
          {mustHaves.length > 0 && (
            <div className="space-y-2">
              <h3 className="text-xs font-extrabold uppercase tracking-widest text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5">
                <CheckCircle className="h-4 w-4 text-emerald-500" /> Must-Have Features (V1 Core)
              </h3>
              <div className="grid gap-3 sm:grid-cols-2">
                {mustHaves.map((item: any, idx: number) => {
                  const name = typeof item === "string" ? item : item.feature;
                  const purpose = typeof item === "string" ? null : item.purpose;
                  return (
                    <div key={idx} className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4 border-l-4 border-l-emerald-500">
                      <div className="text-xs font-bold text-foreground">{name}</div>
                      {purpose && <div className="text-[11px] text-muted-foreground mt-1 leading-relaxed">{purpose}</div>}
                    </div>
                  );
                })}
              </div>
          ...
          {shouldHaves.length > 0 && (
            <div className="space-y-2 pt-2">
              <h3 className="text-xs font-extrabold uppercase tracking-widest text-blue-600 dark:text-blue-400 flex items-center gap-1.5">
                <Layers className="h-4 w-4 text-blue-500" /> Should-Have Features
              </h3>
              <div className="grid gap-3 sm:grid-cols-2">
                {shouldHaves.map((item: any, idx: number) => {
                  ...
                  const purpose = typeof item === "string" ? null : item.purpose;
                  return (
                    <div key={idx} className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-4 border-l-4 border-l-blue-500">
                      <div className="text-xs font-bold text-foreground">{name}</div>
                      {purpose && <div className="text-[11px] text-muted-foreground mt-1 leading-relaxed">{purpose}</div>}
                    </div>
                  );
          ...
          {futureFeatures.length > 0 && (
            <div className="space-y-2 pt-2">
              <h3 className="text-xs font-extrabold uppercase tracking-widest text-amber-600 dark:text-amber-400 flex items-center gap-1.5">
                <Lightbulb className="h-4 w-4 text-amber-500" /> Nice-to-Have / Future Rollout (V2)
              </h3>
              <div className="grid gap-3 sm:grid-cols-2">
                {futureFeatures.map((item: any, idx: number) => {
                  const name = typeof item === "string" ? item : item.feature;
                  const reason = typeof item === "string" ? null : (item.reason ?? item.purpose);
                  return (
                    <div key={idx} className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4 border-l-4 border-l-amber-500">
                      <div className="text-xs font-bold text-foreground">{name}</div>
          ...
          {excluded.length > 0 && (
            <div className="space-y-2 pt-2">
              <h3 className="text-xs font-extrabold uppercase tracking-widest text-rose-600 dark:text-rose-400 flex items-center gap-1.5">
                <AlertTriangle className="h-4 w-4 text-rose-500" /> Strictly Excluded from MVP Scope
              </h3>
              <div className="flex flex-wrap gap-2">
                {excluded.map((item: any, idx: number) => {
                  const name = typeof item === "string" ? item : (item.feature ?? item.item);
                  return (
                    <span key={idx} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium border border-rose-500/25 bg-rose-500/10 text-rose-700 dark:text-rose-300">
                      {name}
                    </span>
                  );
                })}
      ...
      <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
        ...
        {Object.keys(stack).length > 0 && (
          <div className="space-y-3 mb-6">
            <h3 className="text-xs font-extrabold uppercase tracking-widest text-muted-foreground">Recommended Tech Stack</h3>
            <div className="grid gap-3 grid-cols-2 sm:grid-cols-3">
              {Object.entries(stack).map(([layer, tech]: [string, any]) => {
                const label = layer.replace(/_/g, " ").replace(/\b\w/g, c => c.toUpperCase());
                const techVal = Array.isArray(tech) ? tech.join(", ") : String(tech);
                return (
                  <div key={layer} className="p-3 border border-border bg-background/50 rounded-xl space-y-1">
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{label}</span>
                    <div className="text-xs font-bold text-foreground">{techVal}</div>
                  </div>
                );
              })}
            </div>
        ...
        {components.length > 0 && (
          <div className="space-y-3 mb-6 pt-4 border-t border-border/40">
            <h3 className="text-xs font-extrabold uppercase tracking-widest text-muted-foreground">System Components</h3>
            <div className="space-y-2">
              {components.map((c: any, idx: number) => (
                <div key={idx} className="flex gap-3 p-3 rounded-xl border border-border bg-background/30">
                  <div className="p-1.5 rounded-lg bg-primary/10 border border-primary/15 text-primary shrink-0 self-start">
                    <Layers className="h-3.5 w-3.5" />
                  </div>
                  <div>
                    <div className="text-xs font-bold text-foreground">{c.component}</div>
                    {c.purpose && <div className="text-[11px] text-muted-foreground mt-0.5 leading-relaxed">{c.purpose}</div>}
                  </div>
                </div>
              ))}
            </div>
        ...
        {(apis.length > 0 || dbEntities.length > 0) && (
          <div className="grid gap-4 sm:grid-cols-2 pt-4 border-t border-border/40">
            {apis.length > 0 && (
              <div className="space-y-2">
                <h4 className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Core API Modules</h4>
                <div className="flex flex-wrap gap-1.5">
                  {apis.map((api: string, idx: number) => (
                    <span key={idx} className="px-2.5 py-1 rounded bg-muted border border-border/50 text-[11px] font-mono font-medium text-foreground/80">
                      {api}
                    </span>
                  ))}
                </div>
            ...
            {dbEntities.length > 0 && (
              <div className="space-y-2">
                <h4 className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Database Entities</h4>
                <div className="flex flex-wrap gap-1.5">
      ...
      <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
          ...
          <div className="mb-6 p-4 rounded-xl border border-amber-500/25 bg-amber-500/5">
            ...
            <div className="flex flex-wrap gap-2">
              {minimum.map((role: string, idx: number) => (
                <span key={idx} className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold bg-amber-500/10 border border-amber-500/20 text-amber-800 dark:text-amber-300">
                  {role}
                </span>
              ))}
            </div>
        ...
        {recommended.length > 0 && (
          <div className="space-y-4">
            <h3 className="text-xs font-extrabold uppercase tracking-widest text-muted-foreground mb-3">Recommended Squad & Positions</h3>
            <div className="grid gap-4 sm:grid-cols-2">
              {recommended.map((member: any, idx: number) => {
                ...
                const resps = Array.isArray(member.responsibilities) 
                  ...
                  : typeof member.responsibilities === "string" 
                    ...
                    : [];
                return (
                  <div key={idx} className="p-4 rounded-xl border border-border bg-background/40 hover:border-border/60 transition-colors flex flex-col justify-between">
                    <div>
                      ...
                      {resps.length > 0 && (
                        <ul className="list-disc pl-4 space-y-1.5 text-[11px] text-foreground/80 leading-relaxed">
                          {resps.map((r: string, rIdx: number) => <li key={rIdx}>{r}</li>)}
                        </ul>
                      )}
                    </div>
                  </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
        ...
        {hasCost && (
          <div className="space-y-4 mb-6">
            <h3 className="text-xs font-extrabold uppercase tracking-widest text-muted-foreground flex items-center gap-1">
              <DollarSign className="h-4 w-4 text-primary" /> Cost Estimation & Capital Requirement
            </h3>
            ...
            {costData.major_cost_drivers && costData.major_cost_drivers.length > 0 && (
              <div className="pt-2">
        ...
        {hasRoadmap && (
          <div className="pt-6 border-t border-border/40 space-y-4">
            <h3 className="text-xs font-extrabold uppercase tracking-widest text-muted-foreground flex items-center gap-1">
              <Clock className="h-4 w-4 text-primary" /> Implementation Timeline Roadmap
            </h3>

            <div className="space-y-4 relative pl-4 border-l border-border/60">
              {Object.entries(roadmapData).map(([phaseKey, phaseVal]: [string, any]) => {
                const phaseTitle = phaseKey.replace(/_/g, " ").replace(/\b\w/g, c => c.toUpperCase());
                ...
                return (
                  <div key={phaseKey} className="relative space-y-1">
                    ...
                    <div className="flex flex-wrap items-baseline gap-2">
                      <h4 className="text-xs font-black text-foreground">{phaseTitle}</h4>
                    ...
                    {deliverables.length > 0 && (
                      <ul className="list-disc pl-4 space-y-0.5 text-[11px] text-muted-foreground leading-relaxed">
                        {deliverables.map((del: string, dIdx: number) => <li key={dIdx}>{del}</li>)}
                      </ul>
                    )}
                  </div>
                );
...
function JsonDocumentRenderer({ data, documentType }: { data: any; documentType?: string }) {
  const [activeTab, setActiveTab] = useState("overview");
  const isAnalysis = data.idea_summary !== undefined || data.research_analysis !== undefined || documentType === "business_validation";
    ...
    return (
      <div className="space-y-8 text-foreground font-sans animate-fadeIn text-left">
        ...
        <div className="rounded-xl border border-border bg-card p-6 shadow-sm relative overflow-hidden">
          ...
          <h2 className="text-xs font-extrabold uppercase tracking-widest text-muted-foreground mb-1">Project Idea Summary</h2>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div className="rounded-xl border border-border bg-card p-6 shadow-sm flex flex-col justify-between">
                    ...
                    <div className="relative flex items-center justify-center w-20 h-20 shrink-0">
                      <svg className="w-20 h-20 transform -rotate-90">
                        ...
                        <circle
                          ...
                          strokeDashoffset={2 * Math.PI * 34 * (1 - overallScore / 10)}
                        />
                      </svg>
                      <span className="absolute text-lg font-black text-foreground">{overallScore}</span>
                    </div>
                    <div>
                      <div className="text-xs font-bold text-foreground">Validation Score</div>
                      <div className="text-[10px] text-muted-foreground">Derived from concept clarity, demand, and execution feasibility.</div>
                    </div>
                  </div>
                )}
              </div>
              ...
              {finalVerdict && (
                <div className="border-t border-border/40 pt-4 mt-2">
                  <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">Final Verdict</div>
            ...
            {dimensionalScores && (
              <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
                <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-4">Dimensional Scores</h3>
                <div className="space-y-3.5">
                  {Object.entries(dimensionalScores).map(([key, scoreVal]: [string, any]) => {
          ...
          <div className="space-y-3">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4 space-y-2">
                <h4 className="text-[10px] font-extrabold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">Strengths</h4>
                {swot.strengths && swot.strengths.length > 0 ? (
                  <ul className="list-disc pl-4 space-y-1 text-xs text-foreground/85 leading-relaxed">
                    {swot.strengths.map((s: string, idx: number) => <li key={idx}>{s}</li>)}
                  </ul>
                ) : (
              ...
              <div className="rounded-xl border border-rose-500/20 bg-rose-500/5 p-4 space-y-2">
                <h4 className="text-[10px] font-extrabold uppercase tracking-wider text-rose-600 dark:text-rose-400">Weaknesses</h4>
                {swot.weaknesses && swot.weaknesses.length > 0 ? (
                  <ul className="list-disc pl-4 space-y-1 text-xs text-foreground/85 leading-relaxed">
                    {swot.weaknesses.map((w: string, idx: number) => <li key={idx}>{w}</li>)}
                  </ul>
                ) : (
              ...
              <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-4 space-y-2">
                <h4 className="text-[10px] font-extrabold uppercase tracking-wider text-blue-600 dark:text-blue-400">Opportunities</h4>
                {swot.opportunities && swot.opportunities.length > 0 ? (
                  <ul className="list-disc pl-4 space-y-1 text-xs text-foreground/85 leading-relaxed">
                    {swot.opportunities.map((o: string, idx: number) => <li key={idx}>{o}</li>)}
                  </ul>
                ) : (
                  <p className="text-xs text-muted-foreground/60 italic">No opportunities listed.</p>
                )}
              </div>

              {/* Threats */}
              <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4 space-y-2">
                <h4 className="text-[10px] font-extrabold uppercase tracking-wider text-amber-600 dark:text-amber-400">Threats</h4>
                {swot.threats && swot.threats.length > 0 ? (
                  <ul className="list-disc pl-4 space-y-1 text-xs text-foreground/85 leading-relaxed">
                    {swot.threats.map((t: string, idx: number) => <li key={idx}>{t}</li>)}
                  </ul>
                ) : (
                  <p className="text-xs text-muted-foreground/60 italic">No threats listed.</p>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="space-y-5">
          <h3 className="text-xs font-extrabold uppercase tracking-widest text-muted-foreground">Market Research & Strategy</h3>
          <div className="space-y-4">
            {[
              { key: "market_demand", title: "Market Demand Analysis", icon: <TrendingUp className="h-4 w-4" /> },
              ...
              return (
                <div key={key} className="rounded-xl border border-border bg-card p-5 shadow-sm space-y-2 hover:border-border/60 transition-colors">
                  <h4 className="text-xs font-bold text-foreground/90 flex items-center gap-2 border-b border-border/10 pb-1.5">
                    <span className="p-1 rounded bg-primary/10 text-primary border border-primary/15">{icon}</span>
                    {title}
                  </h4>
        ...
        {[
          { key: "risks", title: "Identified Risks", border: "border-l-rose-500", icon: <AlertTriangle className="h-4 w-4 text-rose-500" /> },
          { key: "suggestions", title: "Strategic Suggestions", border: "border-l-blue-500", icon: <Lightbulb className="h-4 w-4 text-blue-500" /> },
          { key: "assumptions", title: "Key Assumptions", border: "border-l-primary/60", icon: <CheckCircle className="h-4 w-4 text-primary" /> }
            ...
            <div key={key} className="space-y-2.5">
              ...
              <div className="space-y-2">
                {listItems.map((item: string, idx: number) => (
                  <div key={idx} className={`rounded-lg border border-border bg-card/40 p-3.5 text-xs leading-relaxed text-foreground/80 border-l-4 ${border} shadow-sm`}>
                    {item}
                  </div>
                ))}
              </div>
        ...
        {clarifyingQuestions.length > 0 && (
          <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-5 space-y-3">
            <h3 className="text-xs font-extrabold uppercase tracking-widest text-amber-600 dark:text-amber-400 flex items-center gap-1.5">
              <HelpCircle className="h-4 w-4" /> Clarifying Questions / Next Steps
            </h3>
            <p className="text-xs text-muted-foreground">To further refine the validation model, consider clarifying the following points:</p>
            <ol className="list-decimal pl-5 space-y-2 text-xs text-foreground/80 leading-relaxed font-semibold">
              {clarifyingQuestions.map((q: string, idx: number) => <li key={idx}>{q}</li>)}
            </ol>
          </div>
        )}
      </div>
  ...
  const isFullBlueprint = documentType === "business_blueprint" || documentType === "full_business_blueprint" || data.executive_summary !== undefined;
              ...
              {data.problem_definition && (
                <div className="rounded-xl border border-border bg-card p-5 space-y-2">
                  <h4 className="text-xs font-bold text-foreground/90 flex items-center gap-2">
                    <AlertTriangle className="h-4 w-4 text-primary" /> Problem Statement
                  </h4>
                  <p className="text-xs text-foreground/80 leading-relaxed">{data.problem_definition.problem_statement}</p>
                  {data.problem_definition.current_alternatives?.length > 0 && (
                    <div className="pt-2 space-y-1">
                      <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Current Alternatives</span>
                      <ul className="list-disc pl-4 space-y-0.5 text-xs text-foreground/70">
                        {data.problem_definition.current_alternatives.map((alt: string, idx: number) => <li key={idx}>{alt}</li>)}
                      </ul>
                    </div>
                  )}
                </div>
              ...
              {data.target_users?.primary_users?.length > 0 && (
                <div className="space-y-2.5">
                  <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-1.5">
                    <Users className="h-4 w-4 text-primary" /> Target Audience Personas
                  </h4>
                  <div className="grid gap-3 sm:grid-cols-2">
                    {data.target_users.primary_users.map((user: any, idx: number) => (
                      <div key={idx} className="rounded-xl border border-border bg-card p-4 space-y-2">
                        <div className="text-xs font-bold text-foreground">{user.persona}</div>
                        {user.description && <p className="text-[11px] text-muted-foreground leading-relaxed">{user.description}</p>}
                        {user.pain_points?.length > 0 && (
                          <div className="space-y-1">
                            <span className="text-[9px] font-bold text-rose-500 uppercase tracking-wider">Pain Points</span>
                            <ul className="list-disc pl-4 space-y-0.5 text-[10px] text-foreground/75">
                              {user.pain_points.map((pt: string, pIdx: number) => <li key={pIdx}>{pt}</li>)}
                            </ul>
                          </div>
                        )}
                      </div>
              ...
              {data.product_strategy && (
                <div className="rounded-xl border border-border bg-card p-5 space-y-3">
                  <h4 className="text-xs font-bold text-foreground flex items-center gap-1.5">
                    <Sparkles className="h-4 w-4 text-primary" /> Product Strategy & Value Prop
                  </h4>
                  <div>
                    <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider">Core Value Proposition</span>
                    <p className="text-xs text-foreground/80 leading-relaxed mt-0.5">{data.product_strategy.core_value_proposition}</p>
                  </div>
                  {data.product_strategy.competitive_advantage?.length > 0 && (
                    <div>
                      <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider">Competitive Advantage</span>
                      <ul className="list-disc pl-4 space-y-0.5 text-xs text-foreground/75 mt-0.5">
                        {data.product_strategy.competitive_advantage.map((adv: string, idx: number) => <li key={idx}>{adv}</li>)}
                      </ul>
                    </div>
                  )}
                </div>
            ...
            <div className="space-y-6">
              ...
              {data.user_journey && (
                <div className="rounded-xl border border-border bg-card p-5 space-y-4">
                  <h4 className="text-xs font-bold text-foreground flex items-center gap-1.5">
                    <Layers className="h-4 w-4 text-primary" /> User Journeys & Flows
                  </h4>
                  {Object.entries(data.user_journey).map(([flowKey, flowSteps]: [string, any]) => {
                    const flowTitle = flowKey.replace(/_/g, " ").replace(/\b\w/g, c => c.toUpperCase());
                    return (
                      <div key={flowKey} className="space-y-1.5">
                        <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{flowTitle}</span>
                        <div className="flex items-center flex-wrap gap-2">
                          {flowSteps.map((step: string, sIdx: number) => (
                            <div key={sIdx} className="flex items-center gap-2">
                              ...
                              {sIdx < flowSteps.length - 1 && <span className="text-muted-foreground">→</span>}
                            </div>
                          ))}
                        </div>
                      </div>
          ...
          {activeTab === "architecture" && (
            <div className="space-y-6">
              ...
              {data.security_and_compliance && (
                <div className="rounded-xl border border-border bg-card p-5 space-y-3">
                  <h4 className="text-xs font-bold text-foreground flex items-center gap-1.5">
                    <Award className="h-4 w-4 text-primary" /> Security & Compliance Requirements
                  </h4>
                  <div className="grid gap-3 sm:grid-cols-3">
                    {[
                      { label: "Security", list: data.security_and_compliance.security_requirements, color: "border-rose-500/20 bg-rose-500/5" },
                      { label: "Privacy", list: data.security_and_compliance.privacy_requirements, color: "border-blue-500/20 bg-blue-500/5" },
                      { label: "Compliance", list: data.security_and_compliance.compliance_requirements, color: "border-amber-500/20 bg-amber-500/5" }
                    ].map((sec) => (
                      <div key={sec.label} className={`p-4 rounded-xl border ${sec.color} space-y-1.5`}>
                        <span className="text-[10px] font-bold text-foreground uppercase tracking-wider">{sec.label}</span>
                        <ul className="list-disc pl-4 space-y-1 text-[11px] text-muted-foreground">
                          {sec.list?.map((item: string, idx: number) => <li key={idx}>{item}</li>)}
                        </ul>
                      </div>
                    ))}
                  </div>
  ...
  if (documentType === "technical_architecture" || data.recommended_stack !== undefined) {
    return renderTechnicalArchitecture(data);
  }

  if (documentType === "freelancer_hiring_brief" || data.recommended_team !== undefined) {
  ...
  return (
    <div className="space-y-6 text-foreground font-sans text-xs animate-fadeIn text-left">
      <div className="p-4 border border-border bg-card rounded-xl shadow-sm mb-4">
        ...
        <p className="text-xs text-muted-foreground mt-2">Parsed from raw structured data payload.</p>
      </div>
      ...
      <div className="space-y-4">
        {Object.entries(data).map(([key, val]) => {
          ...
          if (typeof val !== "object") {
            return (
              <div key={key} className="p-4 border border-border bg-card rounded-xl shadow-sm space-y-1">
                <h3 className="font-bold text-foreground">{title}</h3>
                <p className="text-xs text-foreground/85 leading-relaxed">{String(val)}</p>
              </div>
            );
          }
          ...
          if (Array.isArray(val) && val.every(item => typeof item !== "object")) {
            return (
              <div key={key} className="p-4 border border-border bg-card rounded-xl shadow-sm space-y-2">
                <h3 className="font-bold text-foreground">{title}</h3>
          ...
          if (Array.isArray(val) && val.every(item => typeof item === "object" && item !== null)) {
            const headers = Array.from(new Set(val.flatMap(item => Object.keys(item))));
            if (headers.length === 0) return null;
            return (
              <div key={key} className="p-4 border border-border bg-card rounded-xl shadow-sm space-y-3 animate-fadeIn">
                <h3 className="font-bold text-foreground">{title}</h3>
                <div className="overflow-x-auto border border-border/40 rounded-lg shadow-sm">
                  <table className="min-w-full divide-y divide-border/30 text-left text-xs">
                    <thead className="bg-muted/40 font-bold text-foreground">
                      <tr>
                        {headers.map(h => (
                          <th key={h} className="px-3 py-2 border-r border-border/10 font-bold capitalize">
                            {h.replace(/_/g, " ")}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody className="divide-y divide-border/20">
                      {val.map((rowItem: any, rIdx: number) => (
                        <tr key={rIdx} className="hover:bg-muted/15 transition-colors">
                          {headers.map(h => (
                            <td key={h} className="px-3 py-2 border-r border-border/10 text-foreground/80">
                              {typeof rowItem[h] === "object" ? JSON.stringify(rowItem[h]) : String(rowItem[h] ?? "")}
                            </td>
                          ))}
                        </tr>
                      ))}
          ...
          if (typeof val === "object" && val !== null) {
            return (
              <div key={key} className="p-4 border border-border bg-card rounded-xl shadow-sm space-y-3">
                <h3 className="font-bold text-foreground">{title}</h3>
                <div className="grid gap-3 md:grid-cols-2">
                  {Object.entries(val).map(([subKey, subVal]) => {
                    const subTitle = subKey.replace(/_/g, " ").replace(/\b\w/g, c => c.toUpperCase());
                    return (
                      <div key={subKey} className="p-3 border border-border/30 bg-background/30 rounded-lg space-y-0.5 animate-fadeIn">
...
function lenientJsonParse(str: string): any {
  ...
  try {
    ...
    try {
      ...
      const summaryMatch = cleaned.match(/"idea_summary"\s*:\s*"((?:[^"\\]|\\.)*)"/);
      if (summaryMatch) {
        result.idea_summary = decodeJsonString(summaryMatch[1]);
      }
      
      const regionMatch = cleaned.match(/"region_used"\s*:\s*"((?:[^"\\]|\\.)*)"/);
      if (regionMatch) {
        result.region_used = decodeJsonString(regionMatch[1]);
      }
      
      const researchMatch = cleaned.match(/"research_analysis"\s*:\s*\{([\s\S]*)$/);
      if (researchMatch) {
        ...
        const scoreMatch = researchContent.match(/"overall_score"\s*:\s*([0-9.]+)/);
        if (scoreMatch) {
        ...
        const dimMatch = researchContent.match(/"dimensional_scores"\s*:\s*\{([\s\S]*?)\}/);
        if (dimMatch) {
          ...
          while ((m = itemRegex.exec(dimContent)) !== null) {
            result.research_analysis.dimensional_scores[m[1]] = parseFloat(m[2]);
          }
        }
      }
      
      const cqMatch = cleaned.match(/"clarifying_questions"\s*:\s*\[([\s\S]*?)\]/);
      if (cqMatch) {
