...
type WizardPhase = "idea" | "validation" | "technical";
...
export default function CreateRoom() {
  ...
  const validateIdea = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(await readApiError(res, "Azure OpenAI failed to validate the business idea"));
      }
      ...
      const msg = err?.message ?? "Failed to validate idea";
      setError(msg);
      toast.error(msg);
    } finally {
      setValidating(false);
  ...
  const downloadValidationPdf = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(await readApiError(res, "Failed to download validation PDF"));
      }
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const link = window.document.createElement("a");
      link.href = url;
      link.download = `business-validation-${sessionData._id}.pdf`;
      window.document.body.appendChild(link);
      link.click();
      link.remove();
      URL.revokeObjectURL(url);
    } catch (err: any) {
      const msg = err?.message ?? "Failed to download validation PDF";
      setError(msg);
      toast.error(msg);
    } finally {
  ...
  const loadTechnicalQuestions = async () => {
    ...
    try {
      ...
      const data = await res.json();
      setMandatoryQuestions(Array.isArray(data.mandatoryQuestions) ? data.mandatoryQuestions : []);
  ...
  const createRoom = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(await readApiError(res, "Azure OpenAI failed to create the room scope"));
      }
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        <div className="mb-8 grid grid-cols-3 gap-2 text-xs">
          {[
            ["idea", "1", "Business idea"],
            ["validation", "2", "Validation"],
            ["technical", "3", "Technical intake"],
        ...
        {phase === "idea" && (
          <div className="space-y-6">
            <div>
              <div className="text-xs text-primary font-medium uppercase tracking-wider mb-2">Phase 1 of 3</div>
              <h1 className="text-3xl font-bold tracking-tight mb-3">Validate the business idea first</h1>
        ...
        {phase === "validation" && analysis && (
          <div className="space-y-7">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <div className="text-xs text-primary font-medium uppercase tracking-wider mb-2">Phase 2 of 3</div>
                <h1 className="text-3xl font-bold tracking-tight mb-3">Business validation result</h1>
        ...
        {phase === "technical" && (
          <div className="space-y-8">
            <div>
              <div className="text-xs text-primary font-medium uppercase tracking-wider mb-2">Phase 3 of 3</div>
              <h1 className="text-3xl font-bold tracking-tight mb-3">Answer simple technical intake questions</h1>
              <p className="text-muted-foreground max-w-2xl">
                These answers help Azure OpenAI create the live room brief, tickets, milestones, roles, risks, and budget estimate.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-border/40">
              <Button variant="outline" className="h-12" onClick={() => setPhase("validation")} disabled={creatingRoom}>
                Back
              </Button>
              <Button className="flex-1 glow-purple h-12 text-base" onClick={createRoom} disabled={creatingRoom}>
                {creatingRoom ? (
                  <span className="flex items-center gap-2">
                    <span className="w-4 h-4 rounded-full border-2 border-white/30 border-t-white animate-spin" />
                    Creating live room...
