...
function SimpleTable({ rows, columns }: { rows: Array<Record<string, unknown>>; columns: string[] }) {
  if (rows.length === 0) return <p className="text-xs text-muted-foreground">Not available</p>;
  return (
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-border/40 text-xs">
          <thead className="bg-muted/40 font-bold uppercase tracking-wider text-muted-foreground/90">
            <tr>
              {columns.map((column) => (
                <th key={column} className="px-4 py-3 text-left font-bold text-[10px] tracking-wider">
                  {humanizeKey(column)}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-border/20">
            {rows.map((row, rowIndex) => (
              <tr key={rowIndex} className="hover:bg-muted/5 transition-colors duration-150">
                {columns.map((column) => (
                  <td key={column} className="px-4 py-3 align-top text-foreground/80 leading-relaxed font-medium">
                    {Array.isArray(row[column]) ? (
                      <div className="flex flex-wrap gap-1.5 mt-0.5">
                        {asStringList(row[column]).map((str, idx) => (
                          <span key={idx} className="inline-flex items-center rounded-md bg-primary/10 border border-primary/20 px-2 py-0.5 text-[9px] font-semibold text-primary tracking-tight">
                            {str}
                          </span>
                        ))}
                      </div>
...
function ReportReader({
  ...
  const getSectionIcon = (id: string) => {
    ...
    if (lowerId.includes("score")) return <Award className="h-4 w-4 shrink-0" />;
    if (lowerId.includes("market")) return <TrendingUp className="h-4 w-4 shrink-0" />;
    if (lowerId.includes("target_users") || lowerId.includes("users")) return <Globe className="h-4 w-4 shrink-0" />;
    if (lowerId.includes("business") || lowerId.includes("revenue") || lowerId.includes("cost") || lowerId.includes("economic")) return <Coins className="h-4 w-4 shrink-0" />;
    if (lowerId.includes("risk") || lowerId.includes("threat") || lowerId.includes("weakness")) return <AlertCircle className="h-4 w-4 shrink-0" />;
    if (lowerId.includes("swot") || lowerId.includes("suggest") || lowerId.includes("recommend") || lowerId.includes("next")) return <Sparkles className="h-4 w-4 shrink-0" />;
    if (lowerId.includes("assumption")) return <CheckCircle className="h-4 w-4 shrink-0" />;
    if (lowerId.includes("architecture") || lowerId.includes("tech")) return <Cpu className="h-4 w-4 shrink-0" />;
    if (lowerId.includes("summary")) return <FileText className="h-4 w-4 shrink-0" />;
    if (lowerId.includes("strategy") || lowerId.includes("plan")) return <Layout className="h-4 w-4 shrink-0" />;
      ...
      <section className="min-w-0 rounded-2xl border border-border/60 bg-gradient-to-br from-card/95 via-card/85 to-background/95 backdrop-blur-xl p-8 shadow-2xl relative overflow-hidden animate-in fade-in duration-300">
        ...
        {selectedSection ? (
            ...
            <div className="border-b border-border/40 pb-5 mb-4">
              ...
              {selectedSection.description && (
                <div className="relative mt-4 p-4 rounded-xl border border-primary/15 bg-primary/5/20 backdrop-blur-sm">
                  <span className="absolute top-2 left-2 text-primary/15 text-4xl font-serif leading-none select-none">“</span>
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center relative z-10">
            ...
            <p className="text-sm font-medium text-muted-foreground">Select a report section to read.</p>
          </div>
        )}
      </section>
    </div>
...
function PremiumLoader({ title, subtitle }: { title: string; subtitle?: string }) {
  const quotes = [
    { text: "The best way to predict the future is to create it.", author: "Peter Drucker" },
    { text: "Ideas are easy. Implementation is everything.", author: "John Doerr" },
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[450px] p-8 rounded-2xl border border-primary/20 bg-card/45 backdrop-blur-md relative overflow-hidden shadow-2xl animate-in fade-in duration-500">
      ...
      <div className="absolute -bottom-16 -right-16 w-32 h-32 bg-blue-500/10 rounded-full blur-3xl animate-pulse" style={{ animationDelay: "1s" }} />
      ...
      <div className="relative flex items-center justify-center w-24 h-24 mb-8">
        ...
        <div className="absolute inset-2 rounded-full border border-blue-500/20 border-b-blue-500 animate-spin" style={{ animationDuration: '1.5s', animationDirection: 'reverse' }} />
        <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center animate-pulse">
          <span className="text-primary text-lg font-bold font-mono">DX</span>
        </div>
      </div>

      <h3 className="text-xl font-semibold text-foreground tracking-tight text-center mb-2 animate-pulse">{title}</h3>
      {subtitle && <p className="text-sm text-muted-foreground text-center max-w-md mb-8">{subtitle}</p>}
      ...
      <div className="border-t border-border/30 pt-6 w-full max-w-lg flex flex-col items-center min-h-[90px]">
        <div className={`transition-all duration-300 transform ${fade ? "opacity-100 translate-y-0 animate-in fade-in" : "opacity-0 translate-y-2"} text-center`}>
          <p className="text-sm italic text-foreground/80 leading-relaxed font-medium">"{currentQuote.text}"</p>
          <p className="text-xs text-primary/70 mt-2 font-mono">— {currentQuote.author}</p>
        </div>
      </div>
    </div>
  );
...
function renderRoadmap(value: unknown) {
  ...
  if (phases.length === 0) return <p className="text-xs text-muted-foreground">Not available</p>;
          ...
          <div key={key} className="relative group">
            ...
            <div className="rounded-xl border border-border/50 bg-gradient-to-br from-card to-background/50 p-5 shadow-sm transition-all duration-200 hover:border-primary/20">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border/30 pb-3 mb-3">
                <div>
                  <span className="text-[9px] text-primary uppercase font-bold tracking-wider">Phase {idx + 1}</span>
                  <h3 className="text-xs font-bold text-foreground">{humanizeKey(key)}</h3>
                </div>
  ...
  return (
    <div className="space-y-6">
      {recommended.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-xs font-bold text-foreground uppercase tracking-tight">Recommended Team Roles</h3>
          <div className="grid gap-4 sm:grid-cols-2">
            {recommended.map((item, idx) => (
              <div key={idx} className="rounded-xl border border-border bg-card p-4 space-y-2 hover:border-primary/10 transition-colors">
                <div className="flex items-center justify-between border-b border-border/40 pb-2">
                  <span className="text-xs font-bold text-foreground">{formatPrimitive(item.role)}</span>
                  <span className="rounded bg-primary/10 border border-primary/20 px-2 py-0.5 text-[9px] font-semibold text-primary uppercase">Required</span>
                </div>
                <p className="text-xs text-muted-foreground leading-relaxed">{formatPrimitive(item.responsibilities)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {minimum.length > 0 && (
        <div className="rounded-xl border border-border/50 bg-background/25 p-4 space-y-2">
          <h3 className="text-xs font-bold text-foreground uppercase tracking-tight">Minimum Viable Team Size</h3>
      ...
      {mustHaves.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-bold text-foreground uppercase tracking-wider">Must-Have Features (Core V1)</h3>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            {mustHaves.map((item, i) => (
              <div key={i} className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4 space-y-2 relative overflow-hidden">
                <div className="absolute top-0 right-0 h-1 w-16 bg-emerald-500" />
                <div className="flex items-start justify-between gap-2">
                  <span className="text-xs font-bold text-foreground">{formatPrimitive(item.feature)}</span>
                  {!!item.priority && (
                ...
                <p className="text-xs text-muted-foreground leading-relaxed">{formatPrimitive(item.purpose)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {shouldHaves.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-bold text-foreground uppercase tracking-wider">Should-Have Features (Next Priority)</h3>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            {shouldHaves.map((item, i) => (
              <div key={i} className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-4 space-y-2">
                <span className="text-xs font-bold text-foreground block">{formatPrimitive(item.feature)}</span>
                <p className="text-xs text-muted-foreground leading-relaxed">{formatPrimitive(item.purpose)}</p>
              </div>
            ))}
          </div>
      ...
      {futureFeatures.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-bold text-foreground uppercase tracking-wider">Future Iterations (V2+)</h3>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            {futureFeatures.map((item, i) => (
              <div key={i} className="rounded-xl border border-border bg-card p-4 space-y-2">
                <span className="text-xs font-bold text-muted-foreground block">{formatPrimitive(item.feature)}</span>
                <p className="text-xs text-muted-foreground leading-relaxed">{formatPrimitive(item.reason)}</p>
              </div>
            ))}
          </div>
        </div>
...
function renderTargetUsers(value: unknown) {
  ...
  return (
    <div className="space-y-6">
      {primary.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-xs font-bold text-foreground uppercase tracking-tight">Primary Target Audience</h3>
          <div className="grid gap-4 md:grid-cols-2">
            {primary.map((item, idx) => (
              <div key={idx} className="rounded-xl border border-primary/20 bg-gradient-to-br from-card to-background p-5 space-y-3 shadow-sm">
                <div className="flex items-center gap-2.5">
                  <div className="h-7 w-7 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-xs">
                    {idx + 1}
                  </div>
                  <div>
                    <span className="text-xs font-bold text-foreground block">{formatPrimitive(item.persona)}</span>
                    <span className="text-[9px] text-muted-foreground uppercase tracking-wider">Primary Persona</span>
                  </div>
                </div>
                <p className="text-xs text-muted-foreground leading-relaxed">{formatPrimitive(item.description)}</p>
                {!!item.pain_points && (
                  <div className="pt-2 border-t border-border/40">
                    <span className="text-[9px] font-bold uppercase text-red-400 block mb-1">Pain Points</span>
                    <p className="text-xs text-muted-foreground leading-relaxed">{formatPrimitive(item.pain_points)}</p>
                  </div>
                )}
                {!!item.goals && (
                  <div className="pt-2 border-t border-border/40">
                    <span className="text-[9px] font-bold uppercase text-emerald-400 block mb-1">Goals</span>
                    <p className="text-xs text-muted-foreground leading-relaxed">{formatPrimitive(item.goals)}</p>
                  </div>
                )}
              </div>
            ))}
      ...
      {secondary.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-xs font-bold text-foreground uppercase tracking-tight">Secondary Audience</h3>
          <div className="grid gap-4 md:grid-cols-2">
            {secondary.map((item, idx) => (
              <div key={idx} className="rounded-xl border border-border bg-background/20 p-4 space-y-2">
                <span className="text-xs font-bold text-foreground block">{formatPrimitive(item.persona)}</span>
                <p className="text-xs text-muted-foreground leading-relaxed">{formatPrimitive(item.description)}</p>
              </div>
            ))}
...
function renderTechnicalArchitecture(value: unknown) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="mb-3 text-xs font-bold text-foreground uppercase tracking-tight">Recommended Stack</h3>
        <KeyValueGrid data={asRecord(architecture.recommended_stack)} />
      </div>
      <div>
        <h3 className="mb-3 text-xs font-bold text-foreground uppercase tracking-tight">System Components</h3>
...
Please return ONLY the modified answer itself, without any introductory or conversational text (no "Here is...", no markdown code blocks), so it can replace the current input.`;
  ...
  const handlePhase1FieldRefine = async (field: keyof Phase1ReviewForm, fieldLabel: string) => {
    ...
    return (
      <div className="space-y-2 rounded-xl border border-border/30 bg-background/20 p-4 transition-all hover:border-border/50">
        ...
        {isTextarea ? (
          ...
          <input
            value={value}
            onChange={(event) => updatePhase1ReviewField(field, event.target.value)}
            placeholder={placeholder}
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <div className="sticky top-0 z-10 border-b border-border/40 bg-background/90 backdrop-blur-sm">
        <div className={`${(phase === "idea" || !isChatOpen) ? "max-w-5xl" : "max-w-[1400px]"} mx-auto px-6 h-14 flex items-center justify-between transition-all duration-300`}>
          <div className="flex items-center gap-3 min-w-0">
            <button onClick={() => navigate("/business/dashboard")} className="text-muted-foreground hover:text-foreground text-sm transition-colors shrink-0">
              Back to dashboard
            </button>
      ...
      <div className={`${(phase === "idea" || !isChatOpen) ? "max-w-5xl" : "max-w-[1400px] grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]"} mx-auto px-6 py-10 transition-all duration-300`}>
        ...
        {phase === "analysis" && analysis && (
          (savingPhase1Review || loadingQuestions) ? (
            <PremiumLoader 
              title={savingPhase1Review ? "Saving Business Review" : "Preparing Blueprint Questions"} 
              subtitle={savingPhase1Review ? "Updating launch session with your confirmed assumptions, target audiences, and market configurations..." : "Formulating targeted mandatory and optional technical scoping questions for your blueprint..."} 
            />
          ) : (
            <div className="space-y-7">
              ...
              <div className="grid gap-6 md:grid-cols-3">
                ...
                <div className="relative overflow-hidden rounded-2xl border border-border/60 bg-gradient-to-br from-card to-background p-6 shadow-md transition-all duration-300 hover:border-primary/30">
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Analysis Verdict</span>
                    {research?.final_verdict && (
                      <span className={`rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-wider shadow-sm ${
                        research.final_verdict.toLowerCase().includes("viable") 
                          ? "bg-green-500/10 text-green-500 border border-green-500/20" 
                          : research.final_verdict.toLowerCase().includes("work") 
                  ...
                  <div className="space-y-2">
                    <h3 className="text-2xl font-bold text-foreground">
                      {research?.final_verdict?.toLowerCase().includes("viable") ? "Strong Potential" : "Needs Refinement"}
                    </h3>
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      {research?.verdict_reasoning ?? "The initial analysis is complete. Review the suggestions and warnings before proceeding."}
                    </p>
                  </div>
                </div>
                ...
                <div className="relative overflow-hidden rounded-2xl border border-border/60 bg-gradient-to-br from-card to-background p-6 shadow-md flex items-center gap-6 transition-all duration-300 hover:border-primary/30">
                  <div className="relative flex items-center justify-center shrink-0 w-24 h-24">
                    <svg className="w-full h-full transform -rotate-90">
                      <circle cx="48" cy="48" r="40" stroke="currentColor" strokeWidth="6" className="text-muted/10" fill="transparent" />
                      <circle 
                        cx="48" 
                        cy="48" 
                        r="40" 
                        ...
                        strokeDashoffset={2 * Math.PI * 40 * (1 - (research?.overall_score ?? 0) / 10)}
                        strokeLinecap="round"
                        fill="transparent" 
                      />
                    </svg>
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <span className="text-2xl font-extrabold font-mono text-foreground leading-none">{research?.overall_score ?? "N/A"}</span>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <h3 className="text-xl font-bold text-foreground">Idea Viability</h3>
                    <p className="text-xs text-muted-foreground">
                      Composite score based on market size, feasibility, demand & competitive advantage.
                    </p>
                  </div>
                </div>
                ...
                <div className="relative overflow-hidden rounded-2xl border border-border/60 bg-gradient-to-br from-card to-background p-6 shadow-md transition-all duration-300 hover:border-primary/30">
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Target Region</span>
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      Analysis calibrated for local demographics, regional regulations, and specific market dynamics.
                    </p>
              ...
              <div className="relative overflow-hidden rounded-2xl border border-primary/20 bg-gradient-to-b from-card/80 to-card/40 p-6 space-y-6 shadow-lg">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border/40 pb-4">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold uppercase tracking-wider text-primary">Business Review</span>
                    ...
                    <h2 className="mt-1 text-xl font-bold text-foreground">Confirm Phase 2 Inputs & Assumptions</h2>
                    <p className="mt-1 max-w-2xl text-xs text-muted-foreground leading-relaxed">
                      We've generated the business requirements based on your idea. Use the fields below to customize, adjust, or use AI to refine specific values before generating the technical blueprint.
                    </p>
                  </div>
                </div>

                <div className="grid gap-6 md:grid-cols-2">
                  {renderPhase1ReviewField("region", "Region", "e.g., India, USA, Global", false)}
                  {renderPhase1ReviewField("targetAudience", "Target Customers", "Who the business will serve", true)}
                </div>
                <div className="grid gap-6 md:grid-cols-2">
                  {renderPhase1ReviewField("businessModel", "Business Model", "Subscription, commission, B2B SaaS, etc.")}
                  {renderPhase1ReviewField("competitors", "Competitor Context", "Known competitors, alternatives, or market category")}
                </div>
                <div className="w-full">
                  {renderPhase1ReviewField("ideaSummary", "Idea Summary", "Short corrected description of the business idea")}
                </div>
                <div className="grid gap-6 md:grid-cols-2">
                  {renderPhase1ReviewField("marketDemand", "Market Demand Notes", "What demand, pain, or opportunity should Phase 2 consider?")}
                  {renderPhase1ReviewField("goToMarket", "Go-to-market Notes", "Launch channel, sales motion, geography-specific GTM")}
                </div>
              </div>
        ...
        {phase === "technical" && (
          generatingBlueprint ? (
            ...
            <div className="space-y-8 animate-in fade-in duration-300">
              <div>
                <div className="text-xs text-primary font-medium uppercase tracking-wider mb-2">Phase 3 intake</div>
              ...
              <div className="space-y-6">
                      ...
                      {value.trim() && isRefineOpen && (
                        <div className="space-y-2 bg-card border border-border/40 rounded-lg p-2.5 transition-all animate-in slide-in-from-top-1 duration-200">
                          <textarea
                            value={refineVal}
                            onChange={(e) => {
                              setRefineInputs({ ...refineInputs, [question._id]: e.target.value });
                              e.target.style.height = "auto";
                              e.target.style.height = `${e.target.scrollHeight}px`;
                            }}
                            ref={(el) => {
                              textareaRefs.current[`refine_${question._id}`] = el;
                              if (el) {
                                el.style.height = "auto";
                                el.style.height = `${el.scrollHeight}px`;
                              }
                            ...
                            onKeyDown={(e) => {
                              if (e.key === "Enter" && !e.shiftKey) {
                                e.preventDefault();
                                handleAiRefine(question._id, question.question);
                              }
                            }}
                          />
                          <div className="flex justify-end border-t border-border/20 pt-2">
                            <Button
                              ...
                              variant="ghost"
                              className="h-7 px-3 text-xs text-primary bg-primary/5 hover:bg-primary/10 gap-1"
                              onClick={() => handleAiRefine(question._id, question.question)}
                              disabled={suggestingId !== null || !refineVal.trim()}
                            >
                              {isSuggestingThis ? (
                                <>
                ...
                {optionalQuestions.length > 0 && (
                  <div className="pt-6 border-t border-border/40 space-y-4">
                    <div>
                      <h2 className="text-base font-bold text-foreground">Optional AI questions</h2>
                      <p className="text-xs text-muted-foreground mt-0.5">These are based on your specific idea. Answer the ones you can.</p>
                    </div>
                    {optionalQuestions.map((question, index) => {
                      const isSuggestingThis = suggestingId === question._id;
                      const value = answers[question._id] || "";
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-border/40">
                ...
                <Button className="flex-1 h-12 text-base font-semibold" onClick={generateBlueprint} disabled={generatingBlueprint}>
        ...
        {phase === "blueprint" && blueprint && (
          (creatingRoom || loadingRecommendations) ? (
            <PremiumLoader 
              title={creatingRoom ? "Assembling Your Live Room" : "Scouting Talent Matches"} 
            ...
            <div className="space-y-7 animate-in fade-in duration-300">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  ...
                  <p className="text-muted-foreground max-w-2xl">
                    This report uses the Phase 2 analysis plus the mandatory and optional Phase 3 answers.
                  </p>
                </div>
                <div className="flex gap-2 shrink-0">
                  ...
                  <Button  onClick={generateTalentRecommendations} disabled={loadingRecommendations || creatingRoom}>
                    {loadingRecommendations ? "Finding talent..." : "Generate phase 4 matches"}
                  </Button>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="shrink-0 text-[8px] rounded-md border border-primary/20 bg-primary/10 px-1.5 py-0.5 text-primary font-bold tracking-wider uppercase">Live context</span>
                </div>
                <p className="text-[10px] text-muted-foreground truncate mt-1">
                  {sessionData?._id ? "Personalized AI Launch Assistant" : "Available after Phase 1 analysis."}
                </p>
              </div>
              <Button
                variant="ghost"
            ...
            {chatMessages.length === 0 && (
              <div className="text-center py-10 px-4">
                ...
                <p className="text-xs font-medium text-muted-foreground/80 leading-relaxed">
                  Ask about validations, warnings, MVP scope, team sizes, tech stacks, or roadmap phases.
                </p>
              </div>
            )}
                ...
                <div key={msg.id} className="flex flex-col space-y-1">
                  {isAi ? (
                    ...
                    <div className="flex flex-col items-end space-y-1 max-w-[85%] ml-auto animate-in slide-in-from-right-2 duration-200">
                      ...
                      <span className="text-[8px] font-bold uppercase tracking-wider text-muted-foreground/75 px-1">You</span>
                    </div>
                  )}
                </div>
              );
          ...
          <div className="shrink-0 border-t border-border/40 p-4 bg-background/40 backdrop-blur-md relative z-10">
            {sessionData?._id && smartSuggestions.length > 0 && (
              <div className="mb-4 space-y-2">
                <div className="flex items-center justify-between gap-2 border-b border-border/10 pb-1">
                  <span className="text-[9px] font-bold uppercase tracking-wider text-muted-foreground/85">Smart Suggestions</span>
            ...
            <div className="relative border border-border/50 rounded-xl bg-background/60 focus-within:border-primary/45 focus-within:ring-1 focus-within:ring-primary/10 transition-all p-2">
              ...
              <div className="flex items-center justify-between border-t border-border/20 pt-2 mt-1 px-1.5">
                <span className="text-[9px] text-muted-foreground/60 font-medium">Press Enter to send</span>
                <Button 
                  size="sm"
                  className="h-7 px-3 text-xs gap-1.5" 
                  onClick={() => askLaunchAi()} 
                  disabled={!sessionData?._id || !chatInput.trim() || aiLoading}
                >
                  {aiLoading ? (
