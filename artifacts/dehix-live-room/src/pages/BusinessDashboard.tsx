      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        {!isLoading && roomList.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 mb-8">
            {[
              { label: "Total Rooms", value: roomList.length, mono: false, icon: <Layers className="h-4 w-4 text-primary" /> },
              { label: "Participants", value: roomList.reduce((s: number, r: any) => s + (r.participantCount ?? 0), 0), mono: false, icon: <Users className="h-4 w-4 text-blue-400" /> },
              { label: "Contracted", value: roomList.filter((r: any) => r.status === "contracted").length, mono: false, icon: <FileCheck className="h-4 w-4 text-emerald-400" /> },
              { label: "Closed", value: pastRooms.length, mono: false, icon: <Archive className="h-4 w-4 text-muted-foreground" /> },
              {
                label: "Total Escrow",
              ...
              >
                ...
                <div className={`text-2xl font-black tracking-tight ${s.mono ? "font-mono text-green-500" : "text-foreground"}`}>
                  {s.value}
                </div>
                {(s as any).sub && (
        ...
        <section className="mb-10">
            ...
            <div className="flex items-center gap-1.5 bg-background/40 p-1 border border-border/40 rounded-lg backdrop-blur-sm">
              {(["all", "active", "contracted", "closed"] as const).map((f) => {
                ...
                return (
                  ...
                  >
                    {f} <span className={`ml-1 text-[10px] font-bold ${isActive ? "text-primary-foreground/90" : "text-muted-foreground/60"}`}>{count}</span>
                  </button>
                );
              })}
            </div>
          ...
          {isLoading ? (
            <div className="grid gap-4">
              {[1, 2].map((i) => (
                <div key={i} className="h-28 rounded-xl bg-card/50 border border-border/40 animate-pulse animate-duration-1000" />
              ))}
            </div>
          ) : roomList.length === 0 ? (
            <div className="rounded-xl border border-dashed border-border/50 p-12 text-center bg-card/20 backdrop-blur-sm">
              ...
              <p className="text-muted-foreground text-sm font-medium mb-4">No active rooms</p>
              <Button size="sm" onClick={() => navigate("/room/create")}>
                  ...
                  >
                    <div className="min-w-0 flex-1 space-y-2">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded border capitalize tracking-wider ${STATUS_COLORS[room.status] ?? ""}`}>
                          {room.status}
                        </span>
                        <span className="text-[10px] font-mono text-muted-foreground/60 border border-border/30 bg-muted/10 rounded px-1.5 py-0.5">{room.roomCode}</span>
                      </div>
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground/80 mt-1 line-clamp-1">{room.rawDescription}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      >
                        {copiedCode === room.roomCode ? (
                          <span className="text-green-500 flex items-center gap-1">✓ Copied</span>
                        ) : (
                          <>
                            <ClipboardCopy className="h-3.5 w-3.5" /> Copy invite code
                          </>
                        )}
                      </button>
                      {room.participantCount > 0 && (
                        <span className="text-[11px] text-muted-foreground/80 font-medium flex items-center gap-1 bg-background/35 px-2 py-1 rounded border border-border/10">
                          <Users className="h-3.5 w-3.5 text-blue-400" /> {room.participantCount} joined
                        </span>
                      )}
                      {room.ticketStats?.total > 0 && (
                        <span className="text-[11px] text-muted-foreground/80 font-medium flex items-center gap-1 bg-background/35 px-2 py-1 rounded border border-border/10">
                          <Ticket className="h-3.5 w-3.5 text-amber-400" /> {room.ticketStats.done}/{room.ticketStats.total} tickets
                        </span>
                      )}
